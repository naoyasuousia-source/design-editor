...
<current-situation>
- グループ移動時、グループ外枠とオレンジ枠が一致しない。（移動前・移動後は一致する）
- 二回目クリック時、個別要素の水色枠・水色ポイントがすぐ出ない。（現在マウスを少し動かすと出るので、二回目クリック時すぐに表示するようにする）
</current-situation
<remarks></remarks>
<permission-to-move>NG</permission-to-move>
</requirement>
...
- **目的**: 2段階選択（個別要素メニュー）への切り替え不全の修正
  ...
  - **変更日時**: 2026-01-04
- **目的**: 2段階選択が「一瞬で個別メニューに切り替わってしまう」問題の修正
  - **変更内容**: `useMoveable.ts` に `wasAlreadySelectedRef` を導入。`MouseDown` 時点での選択状態を記録し、`MouseUp` 時には「既にそのグループが選択済みであった場合のみ」個別要素選択へ切り替えるように判定を強化。これにより、1回目のクリックで確実に既存グループのメニュー（解除等）が表示されるようになった。
  - **変更日時**: 2026-01-04
...
- `MouseDown` と `MouseUp` は対で発火するため、`MouseDown` で状態を変更してしまうと `MouseUp` 時点で「既に変更後の状態」を参照してしまい、2段階の処理を一度に実行してしまうバグが発生する。クリック開始時の状態を別途記録しておく必要がある。
