...
const Navbar: React.FC = () => {
    ...
    return (
        <>
            ...
            {/* ファイル名表示バー（メニューバーの外） */}
            {currentFileHandle && (
                <div className="w-full bg-toolbar/30 border-b border-white/5 px-4 py-1.5 flex items-center justify-end">
                    <p className="text-gray-400 text-right pr-6" style={{ fontSize: '12px' }}>
                        {projectFolderName && <span>{projectFolderName} / </span>}
                        <span className="text-gray-300">{currentFileHandle.name}</span>
                    </p>
                </div>
            )}
        </>
