...
- 2026/01/04 20:55: スクロールバースタイルの調整
  - `index.css` のスクロールバー幅を 16px に拡大。
  - 通常時を濃い色、ホバー時を薄い色に入れ替え、常に存在を認識しやすく調整。
...
- **スクロール機能の再分析**: 拡大時にコンテンツが画面外に押し出されスクロール不能になる問題は、Flexbox の中央寄せ（`items-center justify-center`）の副作用。
- **対策状況**: `margin: auto` による中央寄せに切り替え、用紙周囲にバッファ（`my-16`）を設けることで、拡大時も左上を基点とした適切なスクロール領域が確保されるようになった。また、スクロールバー自体の存在感（幅・色）を強調することで操作性を担保。
