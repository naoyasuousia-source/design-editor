...
- **2%拡大バグの修正（極限精密化）** (2026-01-04): 
    - **小数点精度の維持**: `offsetWidth` ではなく `getBoundingClientRect()` を採用し、ブラウザ内部の小数点ピクセル値を保持。
    - **Content Box 基準の徹底**: ボーダーやパディングによる「計算上の膨らみ」を完全に排除し、画像が描画される純粋な内側領域のみで Ratio を算出。
    - **直前 Ratio 計算**: `handleApply` 実行の瞬間に最新の表示サイズから Ratio を再計算し、ウィンドウリサイズ等の外的要因によるズレを完封。
...
- **2%の拡大ズレの原因**: 100px程度の要素で `offsetW` を使うと、0.5pxの丸めが 0.5% になり、さらにボーダー（2px等）の計算漏れが重なると容易に 2% 程度のズレ・拡大として現れる。
- **解決策としての Content Box**: 常に「画像そのものが占める領域」である Content Box を基準に Ratio を出すことで、装飾（ボーダー）に依存しない普遍的なマッピングが可能になる。
- **トリミング再編集（リセット）の逆算幾何学**:
