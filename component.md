...
<current-situation>修正完了。フックの依存配列に selectionMode 等が不足していたため、個別選択中に別要素をクリックしても「既にグループ選択中」と誤判別されていた問題を解消。これにより、別要素クリック時は確実にグループ選択に戻り、再度クリックするまで個別選択には移行しなくなった。</current-situation>
<remarks></remarks>
...
- **個別選択モードからの復帰ロジックの完全修正（依存配列とフラグ管理の適正化）**
    - 目的: 個別選択中に同じグループの別要素をクリックした際、一瞬グループに戻るものの即座に個別選択（メニュー表示）へ戻ってしまうチャタリング問題を解決するため。
    - 内容:
        - `src/hooks/moveable/useMoveableHandlers.ts`: 
            - `handleCanvasClick` の依存配列に `selectionMode` と `activeSubTarget` を追加。これにより、個別選択への遷移後に MouseDown 処理が古い（groupモードと誤認した）クロージャを参照する問題を解消。
            - 別の要素をクリックした際は `wasAlreadySelectedRef.current = false` を強制し、その後の MouseUp で個別選択に切り替わらないようガードを強化。
    - 日時: 2026-01-04 19:25
...
- **グループ化直後の新グループ自動選択機能の実装**
    ...
    - 内容:
        - `src/hooks/useFloatingMenu.ts`: `handleGroup` 関数内で、グループIDを付与した後に `setAutoSelectId(targets[0].id)` を呼び出すように修正。
        - これにより、`useMoveable.ts` 内の `autoSelectId` 監視 `useEffect` が発動し、グループ全体を選択状態に設定。
    - 日時: 2026-01-04 13:20

- **複数選択時に既存グループのオレンジ枠を常時表示する機能の実装**
    - 目的: 複数選択メニュー（グループ化前）が表示されている間、シフト選択された既存グループの外枠を常時オレンジ枠で表示し、グループの範囲を視覚的に明確にするため。
    - 内容:
        - `src/components/features/workspace/MoveableManager.tsx`: `existingGroupBoundsMap` という新しいメモ化処理を追加。複数選択状態かつ同一グループではない場合（`canGroup` 状態）に、選択要素内の既存グループIDを抽出し、各グループのバウンディングボックスを計算。
        - 計算されたバウンディングボックスをもとに、オレンジ枠オーバーレイ（`existing-group-overlay`）をレンダリング。
    - 日時: 2026-01-04 13:20
...
- **複数要素選択時の意図しないテキスト選択（ハイライト）の防止**
    - 要件: 複数要素選択時、なぜか選択してないテキストボックスも含めて、いろいろなテキストの一部がドラッグ選択されてるように青くなってしまうので、複数要素選択時は、一切テキストが青くならないようにする。
    - 解決方法: `src/styles/index.css` にて `.DesignSurface` に `user-select: none` を適用し、`[contenteditable="true"]` に対して `user-select: text !important` を適用。これにより、編集時のみテキスト選択が可能となり、通常操作時は意図しないテキストハイライトが防止される。
